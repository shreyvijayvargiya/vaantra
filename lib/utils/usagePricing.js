/** One-time starter allowance (minutes) for new accounts — not a monthly reset. */
export const FREE_STARTER_MINUTES = 10;

/** Polar product for pay-per-minute translation credits (USD checkout). */
export const USAGE_POLAR_PRODUCT_ID = "7339f4ea-f891-49a8-956d-25f45371d913";
...
/** Smallest slider index whose pack size is >= `minutes` (for “buy at least this many” UX). */
export function sliderIndexForAtLeastMinutes(minutes) {
	const m = Math.ceil(Math.max(0, Number(minutes) || 0));
	if (m <= 0) return 0;
	for (let i = 0; i < USAGE_MINUTE_STEPS.length; i++) {
		if (USAGE_MINUTE_STEPS[i] >= m) return i;
	}
	return USAGE_MINUTE_STEPS.length - 1;
}
