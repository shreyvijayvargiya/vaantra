...
import { stripFramesForStorage } from "../videoEditorApi";
...
export function buildGroupDocumentForFirestore(group) {
	...
	const jobs = (n.jobs || []).map((j) => {
		const job = {
			...
			durationMinutes:
				j.durationMinutes != null && Number.isFinite(Number(j.durationMinutes))
					? Number(j.durationMinutes)
					: null,
			brandVoiceId: j.brandVoiceId ?? null,
			...
			editorFramesStored:
				j.editorFramesStored ??
				(j.editorFrames ? stripFramesForStorage(j.editorFrames) : null),
			globalStyle: j.globalStyle ?? j.style ?? null,
