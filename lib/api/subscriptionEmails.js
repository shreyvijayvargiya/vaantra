...
<p>Hi ${customerName},</p>
<p>Your payment was successful. We added <strong>${minutes} minutes</strong> of translation time to your aantraa account.</p>
<p>Amount: <strong>${currency.toUpperCase()} ${amt}</strong></p>
<p>Order: <code>${orderId || "—"}</code></p>
<p>Open the app to start translating.</p>
<p style="color:#71717a;font-size:13px;">— aantraa</p>
</body></html>`;
		const data = await resend.emails.send({
			from: process.env.RESEND_FROM_EMAIL || "connect@ihatereading.in",
			to: [customerEmail],
			subject: `aantraa — ${minutes} translation minutes added`,
			html,
		});
