...
function blogToVideoPostUrl() {
	return `${getVideoToolsApiOrigin()}/api/video-editor/generate`;
}

function blogToVideoStatusUrl(id) {
	return `${getVideoToolsApiOrigin()}/api/video-editor/${encodeURIComponent(id)}`;
}
...
export async function startBlogToVideoJob(payload) {
	return generateVideoEditorProject(payload);
}

export async function getBlogToVideoJob(id) {
	return getVideoEditorProject(id);
}
...
export async function pollBlogToVideoJob(id, { onStatus, signal, maxMs = MAX_POLL_MS } = {}) {
	...
	for (;;) {
		...
		if (status === "ready" || status === "failed") {
			if (status === "failed") {
				throw new Error(job?.error || job?.user_message || "Video editor job failed");
			}
			return job;
		}
		if (Date.now() - started > maxMs) {
			throw new Error("Video generation timed out — try again or use a shorter article");
		}
